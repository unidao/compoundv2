const cDAI = artifacts.require("CErc20Delegator");
const Comptroller = artifacts.require("Comptroller");
const DAIInterestRateModel = artifacts.require("DAIInterestRateModel");
const CDaiDelegate = artifacts.require("CDaiDelegate");

const underlying = "0x6b175474e89094c44da98b954eedeac495271d0f";
const mantissa= "200000000000000000000000000";
const name_= "Compound Dai";
const symbol = "cDAI";
const decimals = 8;
const admin = "0x2fd852c9a9aBb66788F96955E9928aEF3D71aE98";
const becomeImplementationData = "";

module.exports = function(deployer) {
  deployer.deploy(cDAI, underlying, Comptroller.address, DAIInterestRateModel.address, mantissa, name_, symbol, decimals, admin, CDaiDelegate.address, becomeImplementationData);
};
